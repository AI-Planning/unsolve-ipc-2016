...
RESOURCE = re.compile(r'\s*\(money level(\d+)\)\s*')
...
def scale_c(prob, scale):
    ...
    f = open(prob)
    new_budget = {}
    m = RESOURCE.search(f.read())
    if m:
        new_budget = int(int(m.group(1)) * scale)
        if new_budget > max_cost:
            max_cost = new_budget
    if scale < 1:
        for l in lines:
...
def main(argv):
    parser = argparse.ArgumentParser(description="Scaling constrainedness of TPP instances. Stores new files in the same directory where the 1.0 problem files are currently stored.")
    parser.add_argument("path", type=str, help="Path to TPP problem files with constrainedness 1.0")
