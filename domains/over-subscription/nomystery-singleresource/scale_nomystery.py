...
MONEY = re.compile(r'\s*\(fuel (t\d+) level(\d+)\)\s*')
    ...
    while curl < len(lines):
        ...
        if where == -1:
            if '(define (problem' in l:
                where += 1
                xxx = l.find('c100')
                newprob += l[:xxx] + 'c%03d' % int(scale * 100) + l[xxx
                    + 4:]
            ...
            where += 1
            curl -= 1
        elif where == 3 and MONEY.search(l):
            lvls = MONEY.search(l)
            old = lvls.group(1)
            newprob += '(fuel %s level%s)\n' % (old, new_budget[old])
        else:
            newprob += l
    return newprob
...
def main(argv):
    parser = argparse.ArgumentParser(description="Scaling constrainedness of NoMystery instances. Stores new files in the same directory where the 1.0 problem files are currently stored.")
    parser.add_argument("path", type=str, help="path to NoMystery problem files with constrainedness 1.0")
