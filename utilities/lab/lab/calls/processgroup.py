...
class ProcessGroup(object):
    ...
    def get_process_times(self):
        return '\n'.join(["%s: %s" % (str(p.total_time() / float(JIFFIES_PER_SECOND)), p.cmdline) for p in self.processes])

    def get_process_mems(self):
        return '\n'.join(["%s: %s" % (str(p.vsize / float(2 ** 20)), p.cmdline) for p in self.processes])
