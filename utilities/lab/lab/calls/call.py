...
class Call(subprocess.Popen):
    def __init__(self, args, name='call', time_limit=1800, mem_limit=2048,
        ...
        *check_interval* is the time in seconds between two queries to the
        ...
        self.JAVA_PLANNER = (len(args) > 1) and ('team1/' in args[1])
        self.MAX_MEM = 0
        ...
        if len(args) < 2: 
            set_property('test_call_py__%s'%str(self.name), str(args))
            subprocess.Popen.__init__(self, args, preexec_fn=prepare_call, **kwargs)
        else:
#            if ('team1/' in args[1]) or \
#               ('team3v1/' in args[1]) or \
#               ('team3v2/' in args[1]) or \
#               ('team4v1/' in args[1]) or \
#               ('team4v2/' in args[1]) or \
#               ('team5/' in args[1]):
            if 'team1/' in args[1]:
                set_property('test_call_py_nomemlim-team_%s'%str(self.name), str(args))
                subprocess.Popen.__init__(self, args, preexec_fn=prepare_call_nomemlim, **kwargs)
            else:
                set_property('test_call_py_other_teams_%s'%str(self.name), str(args))
                subprocess.Popen.__init__(self, args, preexec_fn=prepare_call, **kwargs)
    ...
    def wait(self):
        ...
        while True:
            ...
            self.MAX_MEM = max(self.MAX_MEM, total_vsize)
            set_property('max_memory_measured', self.MAX_MEM)
            ...
            elif total_vsize > self.mem_limit + 512 and not self.JAVA_PLANNER:
                print "\nMems:\n%s\n" % group.get_process_mems()
            ...
            try_kill = (total_time >= self.time_limit + self.kill_delay or
                        ...
                        self.kill_delay or
                        (total_vsize > 1.5 * self.mem_limit and not self.JAVA_PLANNER))
